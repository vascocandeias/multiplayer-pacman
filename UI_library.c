...
int create_board_window(int dim_x, int dim_y) {
  ...
  for (i = 0; i < n_rows + 1; i++) {
    SDL_RenderDrawLine(renderer, 0, i * row_height, screen_width,
                       i * row_height);
  }

  for (i = 0; i < n_cols + 1; i++) {
    SDL_RenderDrawLine(renderer, i * col_width, 0, i * col_width,
                       screen_height);
  }
  SDL_SetRenderTarget(renderer, NULL);
  SDL_RenderCopy(renderer, display, NULL, NULL);
...
void priv_paint_place(int board_x, int board_y, int r, int g, int b,
                      SDL_Texture* pic) {
  SDL_Rect rect;
